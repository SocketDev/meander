 ...
 * publishing to Val Town. Serves files out of the `walkthrough/`
 * directory next to the consumer's `walkthrough.json` and
 * rewrites path requests to match the URL shape the deployed
 * val uses:
 *
 *   /                        → walkthrough/index.html
 *   /:slug                   → walkthrough/index.html
 *   /:slug/part/:n           → walkthrough/walkthrough-part-:n.html
 *   /:slug/documents         → walkthrough/documents.html
 *   /:slug/<asset>           → walkthrough/<asset>
 *   /<asset>                 → walkthrough/<asset>
 *
...
function routeToFile(
  ...
  if (partMatch) {
    ...
    if (partIds.has(n)) {
      return `walkthrough-part-${n}.html`;
    }
    return undefined;
  }
  // /:slug/documents
  ...
  const asset = clean.startsWith(slugPrefix) ? clean.slice(slugPrefix.length) : clean;
  return asset;
}
 ...
 */
async function readWalkthroughMeta(outDir: string): Promise<{
  slug: string;
  partIds: Set<number>;
  hasDocuments: boolean;
}> {
...
export async function serve(
  ...
  /* Treat the directory that HOLDS walkthrough.json as the
  ...
  // Conventional output dir that `generate` writes to
  ...
  const server = createServer(async (req, res) => {
    ...
    // Path traversal guard — target must stay inside outDir
