...
file covers what the findings mean and how to fix each class
...
workflow actually uses. The allow list is a security boundary, not
a convenience list.
...
Fix: pin MCP server URLs to literal origins. Wrap stdio servers in
a binary, not a shell invocation. Require auth for any non-local
MCP.
...
### "Hardcoded token" in CLAUDE.md token-hygiene block

CLAUDE.md's token-hygiene section enumerates *forbidden token
prefixes* (the patterns the hook blocks: `vtwn_`, `ghp_`, `sk-…`,
JWTs, etc.). AgentShield's pattern match finds those prefixes.
That's a doc listing what to block, not a leaked token.

Check: if the match is in prose documenting the block list, it's a
false positive.

Resolution: either leave it (it's illustrative) or wrap the pattern
in `<code>` inline code blocks so AgentShield classifies it as
content-not-value.
...
isn't a full SHA. Same rule as for regular actions. Pin the SHA.

Resolution: pin the SHA with a trailing `# main` (or version tag)
comment.
...
Resolution: the zizmor ignore comment is `# zizmor:
ignore[template-injection] trusted-field` on the line above the
interpolation.
...
```json
{
  "env": { "VALTOWN_API_TOKEN": "<literal-token-here>" }
}
```
...
```json
{
  "env": { "VALTOWN_API_TOKEN": "${VALTOWN_API_TOKEN}" }
}
```

Shell-style substitution reads from the environment at skill-run
time; the literal token never lands in the repo. The
`token-hygiene` hook would also block any command that tried to
emit the literal value, but the AgentShield scan catches it at
rest in the file.
...
Workflow-wide suppression lives in `.github/zizmor.yml` under
`rules:`. Meander already disables `secrets-outside-env` there;
only add to that list with a comment explaining *why*.

### Blanket skip for a tool

If zizmor is not installed locally, SKILL.md Phase 3 skips zizmor
with a warning. This is correct for developer machines; CI must
have zizmor installed (via the `.github/actions/setup-and-install`
composite) and skip-with-warning must not occur there. If you see
a CI skip, fix the setup composite.
...
1. Grade + summary at the top. If `A` or `B`, skim the rest for
   ...
   release-blocking.
2. Critical findings. Address every one. These shouldn't ship.
3. High findings. Address unless you have an explicit suppression
   with a reason.
4. Medium / Low. Address if time permits; triage for a follow-up
   PR if not.
