...
Shared output format for meander's scan and review pipelines.
...
When a skill completes as part of a larger pipeline (e.g. quality-scan inside a release flow), output a structured handoff block:
...
If the pipeline tracks state in `.claude/ops/queue.yaml`, update on the final phase:
